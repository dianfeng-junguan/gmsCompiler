use std::{fmt::{Debug, Display}, vec::Vec};
...
static OPERATORS: [(OperatorType,&str);11] = [
    (OperatorType::Plus, "+"),
    (OperatorType::Minus, "-"),
...
/// # Returns
/// 扫描到的字符数
fn scan(toscan:&str, allowed:&str, maxlen:usize) -> usize {
    let mut ptr:usize=0;
...
fn main() {
    ...
    while startptr<rawstr.len() {
        ...
        if LETTERS_ALLOWED.contains(firstchar) {
            let wordlen=scan(&rawstr[startptr..], LETTERS_ALLOWED, rawstr.len()-startptr);
            ...
            let wordlen=scan(&rawstr[startptr..], DIGITS_ALLOWED, rawstr.len()-startptr);
            let word=&rawstr[startptr..startptr+wordlen];
            tokens.push(Token::new(TokenType::Constant(ConstantType::Integer), word));
            startptr+=wordlen;
            pioneerptr=startptr;
        }else if OPERATORS_ALLOWED.contains(firstchar) {
            let wordlen=scan(&rawstr[startptr..], OPERATORS_ALLOWED, 2);
            let word=&rawstr[startptr..startptr+wordlen];
            //检查是哪种操作符
            ...
            let wordlen=scan(&rawstr[startptr..], SEPARATORS_ALLOWED, 1);
            let word=&rawstr[startptr..startptr+wordlen];
            //检查是哪种分隔符
    ...
    for token in tokens_no_whitespace {
        println!("{:?}", token);
    }
}
